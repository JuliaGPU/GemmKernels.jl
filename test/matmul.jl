...
@testset "Matmul API" begin
    ...
    @testset "WMMA Complex GEMM ($( !conjugate_a ? 'N' : 'C' )$( !conjugate_b ? 'N' : 'C' ))" for conjugate_a = [false, true],
        ...
        @testset "(M = $M, N = $N, K = $K)" for M in [128, 256],
            ...
            expected = new_a_h * new_b_h + c_h
            got = Array(d)

            for ind in eachindex(got)
                if !isapprox(got[ind], expected[ind]; rtol=sqrt(eps(Float16)))
                    println("Mismatch at index $(ind): got $(got[ind]), but expected $(expected[ind])")
                end
            end
        end
    end
